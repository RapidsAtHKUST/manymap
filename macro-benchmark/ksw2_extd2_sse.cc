...
#define __dp_code_block1 \
	...
	xt1 = _mm512_loadu_si512(&x8[t8]); \
	vt1 = _mm512_loadu_si512(&v8[t8]); \
	a = _mm512_add_epi8(xt1, vt1); \
	ut = _mm512_load_si512(&u[t]); \
	b = _mm512_add_epi8(_mm512_load_si512(&y[t]), ut); \
	x2t1 = _mm512_loadu_si512(&x28[t8]); \
	a2= _mm512_add_epi8(x2t1, vt1); \
	b2= _mm512_add_epi8(_mm512_load_si512(&y2[t]), ut);
#else
#define __dp_code_block1 \
	z = simd_funcw(load)(&s[t]); \
	xt1 = simd_funcw(loadu)((SIMD_INT*)&x8[t8]);                        /* xt1 <- x[r-1][t..t+15] */ \
	vt1 = simd_funcw(loadu)((SIMD_INT*)&v8[t8]);                        /* vt1 <- v[r-1][t..t+15] */ \
	...
	x2t1= simd_funcw(loadu)((SIMD_INT*)&x28[t8]); \
...
#define __dp_code_block2 \
	...
	simd_funcw(storeu)((SIMD_INT*)&v8[t8], simd_func(sub_epi8)(z, ut)); /* v[r][t..t+15] <- z - u[r-1][t..t+15] */ \
	...
	int r, t, qe = q + e, n_col_, *off = 0, *off_end = 0, tlen_, qlen_, wl, wr, max_sc, min_sc, long_thres, long_diff;
	...
	qlen_ = (qlen + SIMD_WIDTH - 1) / SIMD_WIDTH + 2;
	...
	mem = (uint8_t*)kcalloc(km, tlen_ * 5 + qlen_ * 4 + 1, SIMD_WIDTH);
	u = (SIMD_INT*)(((size_t)mem + SIMD_WIDTH - 1) >> SIMD_SHIFT << SIMD_SHIFT); // 16-byte aligned
	v = u + tlen_, x = v + qlen_, y = x + qlen_, x2 = y + tlen_, y2 = x2 + qlen_;
	s = y2 + tlen_, sf = (uint8_t*)(s + tlen_), qr = sf + tlen_ * SIMD_WIDTH;
	memset(u,  -q  - e,  tlen_ * SIMD_WIDTH);
	memset(v,  -q  - e,  qlen_ * SIMD_WIDTH);
	memset(x,  -q  - e,  qlen_ * SIMD_WIDTH);
	memset(y,  -q  - e,  tlen_ * SIMD_WIDTH);
	memset(x2, -q2 - e2, qlen_ * SIMD_WIDTH);
	...
	for (r = 0; r < qlen + tlen - 1; ++r) {
		...
		int adj = (qlen_ - 1) * SIMD_WIDTH - r - 1;
		int8_t *u8 = (int8_t*)u, *v8 = (int8_t*)v + adj, *x8 = (int8_t*)x + adj, *x28 = (int8_t*)x2 + adj;
		// find the boundaries
		...
		if (!with_cigar) { // score only
			for (t = st_; t <= en_; ++t) {
				...
				simd_funcw(storeu)((SIMD_INT*)&x8[t8],  simd_func(sub_epi8)(simd_func(max_epi8)(a,  zero_), qe_));
				simd_funcw(store)(&y[t],  simd_func(sub_epi8)(simd_func(max_epi8)(b,  zero_), qe_));
				simd_funcw(storeu)((SIMD_INT*)&x28[t8], simd_func(sub_epi8)(simd_func(max_epi8)(a2, zero_), qe2_));
				...
				_mm_storeu_si128((SIMD_INT*)&x8[t8],  _mm_sub_epi8(_mm_and_si128(tmp, a),  qe_));
				tmp = _mm_cmpgt_epi8(b, zero_);
				_mm_store_si128(&y[t],  _mm_sub_epi8(_mm_and_si128(tmp, b),  qe_));
				tmp = _mm_cmpgt_epi8(a2, zero_);
				_mm_storeu_si128((SIMD_INT*)&x28[t8], _mm_sub_epi8(_mm_and_si128(tmp, a2), qe2_));
			...
			for (t = st_; t <= en_; ++t) {
				...
				_mm512_storeu_si512(&x8[t8],  _mm512_sub_epi8(_mm512_max_epi8(a,  zero_), qe_));
				...
				_mm512_storeu_si512(&x28[t8], _mm512_sub_epi8(_mm512_max_epi8(a2, zero_), qe2_));
				...
				simd_funcw(storeu)((SIMD_INT*)&x8[t8],  simd_func(sub_epi8)(simd_funcw(and)(tmp, a),  qe_));
				...
				simd_funcw(storeu)((SIMD_INT*)&x28[t8], simd_func(sub_epi8)(simd_funcw(and)(tmp, a2), qe2_));
			...
			for (t = st_; t <= en_; ++t) {
				...
				simd_funcw(storeu)((SIMD_INT*)&x8[t8],  simd_func(sub_epi8)(simd_funcw(andnot)(tmp, a),  qe_));
				...
				simd_funcw(storeu)((SIMD_INT*)&x28[t8], simd_func(sub_epi8)(simd_funcw(andnot)(tmp, a2), qe2_));
		...
		// for (t = st0; t <= en0; ++t) printf("(%d,%d)\t(%d,%d,%d,%d)\t%d\n", r, t, ((int8_t*)u)[t], ((int8_t*)v8)[0], ((int8_t*)x8)[0], ((int8_t*)y)[t], H[t]); // for debugging
	}
	kfree(km, mem);
