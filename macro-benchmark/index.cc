...
mm_idx_t *mm_idx_load(mm_idx_reader_t *r)
{
	char magic[4];
	uint32_t x[5], i;
	uint64_t sum_len = 0;
	...
	void *addr = mmap(NULL, (size_t)(r->idx_size), PROT_READ, MAP_FILE | MAP_PRIVATE | MAP_POPULATE, r->fp.idx, 0);
	if (addr == MAP_FAILED) {
	...
	if (!memcpy(magic, addr, 4)) return 0; addr += 4;
	if (strncmp(magic, MM_IDX_MAGIC, 4) != 0) return 0;
	if (!memcpy(x, addr, 20)) return 0; addr += 20;
	...
	for (i = 0; i < mi->n_seq; ++i) {
		...
		memcpy(&l, addr, 1); addr += 1;
		if (l) {
			s->name = (char*)kmalloc(mi->km, l + 1);
			memcpy(s->name, addr, l); addr += l;
			s->name[l] = 0;
		}
		memcpy(&s->len, addr, 4); addr += 4;
		s->offset = sum_len;
		sum_len += s->len;
	}
	for (i = 0; i < 1<<mi->b; ++i) {
		...
		khint_t k;
		idxhash_t *h;
		memcpy(&b->n, addr, 4); addr += 4;
		b->p = (uint64_t*)malloc(b->n * 8);
		memcpy(b->p, addr, 8 * (b->n)); addr += 8 * (b->n);
		memcpy(&size, addr, 4); addr += 4;
		if (size == 0) continue;
		b->h = h = kh_init(idx);
	...
	if (!(mi->flag & MM_I_NO_SEQ)) {
		...
		memcpy(mi->S, addr, 4 * (sum_len + 7) / 8); addr += 4 * (sum_len + 7) / 8;
	}
	munmap(initaddr, (size_t)(r->idx_size));
	return mi;
}
...
mm_idx_reader_t *mm_idx_reader_open(const char *fn, const mm_idxopt_t *opt, const char *fn_out)
{
	...
	if (r->is_idx) {
		r->fp.idx = open(fn, O_RDONLY);
		struct stat st;
		stat(fn, &st);
		r->idx_size = st.st_size;
...
{
	return r->is_idx? (r->idx_size) : mm_bseq_eof(r->fp.seq);
}
