...
// #define NSTREAM 128
// #define NSTREAM 112 // 8k
// #define NSTREAM 32 // 16k
#define NSTREAM 8 // 32k

// #define MAXMEM 2100000 // 1k 2016000
// #define MAXMEM 8100000 // 2k, 8032000 
// #define MAXMEM 32100000 // 4k, 32064000
// #define MAXMEM 129000000UL // 8k, 128128000+64000
// #define MAXMEM 513000000UL // 16k, 512256000+128000
#define MAXMEM 2050000000UL // 32k, 2048512000+256000
...
__global__ void ksw_extd2_kernel(void *gm, int qlen, const uint8_t *query, int tlen, const uint8_t *target,
	...
	int t, r, n_col_, last_st, last_en;
	...
	for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
		...
		int en0 = st + (en - st + elementskip) / elementskip * elementskip;
		...
		for (t = st + tid; t < en0; t += elementskip) {
			int8_t d, z, a, b, a2, b2, ut, xt1, x2t1, vt1;
			...
			if (t <= en) {
				x1 = x[t + elementskip - 1];
				vt1 = (tid == 0) ? v1 : v[t - 1];
			}
			__syncthreads();

			if (t <= en) {
				v1 = v[t + elementskip - 1];
				x2t1 = (tid == 0) ? x21 : x2[t - 1];
			}
			__syncthreads();

			if (t <= en) {
				x21 = x2[t + elementskip - 1];
