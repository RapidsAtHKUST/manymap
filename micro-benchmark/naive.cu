...
__global__ void ksw_extd2_kernel(void *gm, int qlen, const uint8_t *query, int tlen, const uint8_t *target,
	...
	u = (int8_t*)(((size_t)gm + WIDTH - 1) >> SHIFT << SHIFT);
	...
	for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
		...
		for (t = st + tid; t < en0; t += elementskip) {
			int8_t z, a, b, a2, b2, ut, xt1, x2t1, vt1;
...
*/
	for (int i = 0; i < nkernel; ++i) {
		...
		ksw_extd2_kernel<<<nblock, nthread, 0, streams[sid]>>>((void*)((int8_t*)d_gm + sid * MAXMEM), qlen, &d_query[sid * MAXLEN], tlen, &d_target[sid * MAXLEN], q, e, q2, e2, w, zdrop, &d_ez[i]);
